...
class TranAD(nn.Module):
	def __init__(self, feats):
		...
		self.lr = lr
		self.batch = 128
		self.n_feats = feats	#多维时序的维度
		self.n_window = 10	#滑动窗口的大小
		self.n = self.n_feats * self.n_window	#多维滑窗大小
		self.pos_encoder = PositionalEncoding(2 * feats, 0.1, self.n_window)	#位置编码
		encoder_layers = TransformerEncoderLayer(d_model=2 * feats, nhead=feats, dim_feedforward=16, dropout=0.1)	#编码器层
		self.transformer_encoder = TransformerEncoder(encoder_layers, 1)
		decoder_layers1 = TransformerDecoderLayer(d_model=2 * feats, nhead=feats, dim_feedforward=16, dropout=0.1)	#第一个解码器，用于Phase 1
		self.transformer_decoder1 = TransformerDecoder(decoder_layers1, 1)
		decoder_layers2 = TransformerDecoderLayer(d_model=2 * feats, nhead=feats, dim_feedforward=16, dropout=0.1)	#第二个解码器，用于Phase 2
		self.transformer_decoder2 = TransformerDecoder(decoder_layers2, 1)
		self.fcn = nn.Sequential(nn.Linear(2 * feats, feats), nn.Sigmoid())	#全连接层，作为解码器的输出层

	# 这是一个辅助函数，封装了数据进入编码器之前的预处理步骤
	def encode(self, src, c, tgt):
		src = torch.cat((src, c), dim=2) #将原始输入 src 和一个名为 c 的张量沿特征维度拼接。这个 c 在第一阶段是全零，在第二阶段是上一阶段的异常分数
		src = src * math.sqrt(self.n_feats) # 这是 Transformer 中的一个标准化步骤，用于缩放输入，防止梯度过小
		src = self.pos_encoder(src)	#用上面初始化好的位置编码器，为拼接后的数据添加位置信息
		memory = self.transformer_encoder(src) # 用上面初始化好的 Transformer 编码器对输入进行编码，得到 memory
		tgt = tgt.repeat(1, 1, 2) # 将目标张量 tgt 沿特征维度重复两次，以匹配编码器的输出维度
		return tgt, memory

	# 这是模型最核心的部分，定义了数据从输入到输出的完整流程
	def forward(self, src, tgt):
		# Phase 1 - Without anomaly scores
		c = torch.zeros_like(src)	# 在第一阶段，创建一个和输入 src 形状完全一样、但数值全为 0 的张量 c
		x1 = self.fcn(self.transformer_decoder1(*self.encode(src, c, tgt)))	# 1.先调用encode进行编码；2.编码结果和tgt一起传递第一个decoder解码；3.解码结果传入全连接层，得到Phase 1的重建结果
		# Phase 2 - With anomaly scores
		c = (x1 - src) ** 2	#计算第一阶段重建结果 x1 和原始输入 src 之间的平方差c
		x2 = self.fcn(self.transformer_decoder2(*self.encode(src, c, tgt))) # 1.再次调用encode进行编码；2.编码结果和c一起传递第二个decoder解码；3.解码结果传入全连接层，得到Phase 2的重建结果
		return x1, x2	#返回两个阶段的重建结果 x1 和 x2
